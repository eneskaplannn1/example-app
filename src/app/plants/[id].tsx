...
import { getUserPlantById } from '../../feature/plants/services/userPlantService';
...
export default function PlantDetailPage() {
  ...
  const loadPlant = async () => {
    try {
      setIsLoading(true);
      const plantData = await getUserPlantById(id);
      setPlant(plantData);
    } catch (error) {
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <ScrollView className="flex-1 px-4 py-4">
        ...
        <View className="mb-6 rounded-lg bg-white p-4 shadow-sm">
          ...
          <TouchableOpacity
            className={`rounded-lg p-3 ${needsWatering ? 'bg-blue-500' : 'bg-green-500'}`}>
            <Text className="text-center font-medium text-white">
              {needsWatering ? 'Water Plant' : 'Plant is Healthy'}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
